...
void main()
{
	...
	vs_out.v_FragPos = vec3(u_ModelMatrix * a_InstanceTransform * vec4(a_Position, 1.0));
	vs_out.v_FragPosLightSpace = u_LightSpaceMatrix * vec4(vs_out.v_FragPos, 1.0);
	gl_Position = u_ProjectionMatrix * u_ViewMatrix * u_ModelMatrix * a_InstanceTransform * vec4(a_Position, 1.0);
}
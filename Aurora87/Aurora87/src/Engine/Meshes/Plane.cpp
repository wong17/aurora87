...
namespace Engine
{
	std::shared_ptr<Mesh> Plane::GenerateMesh() const
	{
		...
		for (size_t i = 0; i < vertices.size(); i += 8) {
			...
			vertices[i + 6] *= m_Size.x; // tx
			vertices[i + 7] *= m_Size.y; // ty
		}
		...
		else if (!m_TexturePath.empty())
		{
			...
			if (auto tex = TextureManager::Get().Load(m_TexturePath, spec))
				...
				std::cerr << "Plane::GenerateMesh(): Error al cargar la textura " << m_TexturePath << "\n";
		}

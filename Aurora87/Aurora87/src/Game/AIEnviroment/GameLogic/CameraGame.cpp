...
namespace Engine {
	...
	CameraGame::CameraGame(int width, int height, glm::vec3 position)
	{
		CameraGame::width = width;
		CameraGame::height = height;
		Position = position;
		playerID = -1;
		count = 0;
	...
	void CameraGame::Matrix(float FOVdeg, float nearPlane, float farPlane)
	{
	...
	void CameraGame::Inputs(GLFWwindow* window, int id)
	{
		playerID = id;
